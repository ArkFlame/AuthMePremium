...
/**
 * BungeeCord Floodgate hook to detect Bedrock (Floodgate) players and interact with Floodgate API.
 */
public class FloodgateHook {
     ...
     * If Floodgate is not installed or not yet available, floodgateApi will be null.
     */
    public FloodgateHook() {
        if (AuthMePremiumPlugin.getInstance().isFloodgateEnabled() && BungeeCord.getInstance().getPluginManager().getPlugin("floodgate") != null) {
     ...
     * @return true if the player is authenticated via Floodgate (Bedrock), false otherwise
     */
